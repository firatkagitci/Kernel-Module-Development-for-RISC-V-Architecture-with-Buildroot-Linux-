...
The driver itself is not enough for the communication between the user and the device, that is why in addition to the driver we need to develop a user program so that we can share data between the user and the device driver with ease. User programs are not able to write to device's memory regions, that is why inside the driver we have used `copy from user` and `copy to user` functions for data transfer, since the driver is able to access the memory regions of the device. 
